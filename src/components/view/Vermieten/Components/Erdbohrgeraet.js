...
class Erdbohrgeraet extends Component{
  constructor(props){
    super(props)
    this.state = {
       redirect: false,
...
onDrop(imageFiles) {
 ...
 artikelHochladen(event) {
   ...
   event.preventDefault();
    ...
    const alert = "Gebe Auskunft über den Motor"
    this.setState({alert: alert, showAlert: true})
    return 0
    }
    if (this.bohrdurchmesserInput.value == "") {
    const alert = "Gebe Auskunft über die Bohrdurchmesser"
    this.setState({alert: alert, showAlert: true})
    return 0
    }
    ...
    this.setState({
      ...
      imageUpload: false,
        ...
        const db = firebase.database().ref('app').child('cards').child('erdbohrgeraet');
        const userId = this.props.user;
        const titel = this.titelInput.value;
        const hersteller = this.herstellerInput.value;
        const gewicht = this.GewichtdesArtikelsInput.value;
        const motor = this.motorInput.value;
        const bohrdurchmesser = this.bohrdurchmesserInput.value;
                            ...
                            <div className=" full-detail mrg-bot-25 padd-bot-30 padd-top-25">
                              ...
                              <form onSubmit={this.artikelHochladen.bind(this)}>
                                ...
                                <div className="row mrg-r-10 mrg-l-10">
                                  ...
                                  <div className="col-sm-6">
                                    <label>Motor</label>
                                    <input type="text" className="form-control" ref={(input) => { this.motorInput = input}} placeholder="Bsp: Benzin. 1:25 Gemisch"/>
                                  </div>
