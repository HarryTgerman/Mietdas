...
class Raupenbagger extends Component{
  constructor(props){
    super(props)
    this.state = {
       redirect: false,
...
onDrop(imageFiles) {
 ...
 artikelHochladen(event) {
   ...
   event.preventDefault();
    ...
    const alert = "Gebe Auskunft über die Grabtiefe"
    this.setState({alert: alert, showAlert: true})
    return 0
    }
    ...
    this.setState({
      ...
      imageUpload: false,
        ...
        const db = firebase.database().ref('app').child('cards').child('raupenbagger');
